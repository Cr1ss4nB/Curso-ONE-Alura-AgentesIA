...
def recibe_linea_devuelve_json(linea):
    respuesta_de_llm = client_openai.chat.completions.create(
        ...
        messages = [
            ...
            Recibirás una línea de texto que es una reseña de una aplicación en un marketplace
            online.
            ...
            Ejemplo de entrada:
            '8794859375Pedro Silva$J'aimais bien ChatGPT. Mais la derniÃ¨re mise Ã  jour a tout
            gÃ¢chÃ©. Elle a tout oubliÃ©.'
            Ejemplo de salida:

            {
            "usuario": "Pedro Silva",
            "reseña_original": "J'aimais bien ChatGPT. Mais la dernière mise à jour a tout gâché. Elle a tout oublié",
            "reseña_es": "Me gustaba mucho ChatGPT. Pero la última actualización lo arruinó
            todo. Olvidó todo",
            "evaluacion": "Negativa"
            }

            Regla importante: debes retornar solo el JSON, sin ningún otro texto además del
            JSON.
            """},
    ...
    respuesta = respuesta_de_llm.choices[0].message.content.replace("```json", "").replace("```", "").strip()
    print(respuesta)
    return respuesta